...
shutdown_rooms(Host) ->
  MyHost = gen_mod:get_module_opt_host(Host, mod_muc,
    <<"conference.@HOST@">>),
  Rooms = mnesia:dirty_select(muc2_online_room,
    [{#muc2_online_room{name_host = '$1',
      pid = '$2'},
      [{'==', {element, 2, '$1'}, MyHost},
        {'==', {node, '$2'}, node()}],
      ['$2']}]),
...
%% Else use the passed options as defined in mod_muc2_room.
...
store_room(_LServer, Host, Name, Opts, mnesia) ->
  F = fun () ->
    mnesia:write(#muc2_room{name_host = {Name, Host},
      opts = Opts})
      end,
  mnesia:transaction(F);
store_room(_LServer, Host, Name, Opts, riak) ->
  {atomic, ejabberd_riak:put(#muc2_room{name_host = {Name, Host},
    opts = Opts},
    muc2_room_schema())};
store_room(LServer, Host, Name, Opts, odbc) ->
  ...
  F = fun () ->
    odbc_queries:update_t(<<"muc2_room">>,
      [<<"name">>, <<"host">>, <<"opts">>],
      [SName, SHost, SOpts],
      [<<"name='">>, SName, <<"' and host='">>,
        SHost, <<"'">>])
...
restore_room(_LServer, Host, Name, mnesia) ->
  case catch mnesia:dirty_read(muc2_room, {Name, Host}) of
    [#muc2_room{opts = Opts}] -> Opts;
    _ -> error
  end;
restore_room(_LServer, Host, Name, riak) ->
  case ejabberd_riak:get(muc2_room, muc2_room_schema(), {Name, Host}) of
    {ok, #muc2_room{opts = Opts}} -> Opts;
    _ -> error
  end;
restore_room(LServer, Host, Name, odbc) ->
  ...
  case catch ejabberd_odbc:sql_query(LServer,
    [<<"select opts from muc2_room where name='">>,
...
forget_room(LServer, Host, Name, mnesia) ->
  ...
  F = fun () -> mnesia:delete({muc2_room, {Name, Host}})
      end,
  mnesia:transaction(F);
forget_room(LServer, Host, Name, riak) ->
  remove_room_mam(LServer, Host, Name),
  {atomic, ejabberd_riak:delete(muc2_room, {Name, Host})};
forget_room(LServer, Host, Name, odbc) ->
  remove_room_mam(LServer, Host, Name),
  SName = ejabberd_odbc:escape(Name),
  SHost = ejabberd_odbc:escape(Host),
  F = fun () ->
    ejabberd_odbc:sql_query_t([<<"delete from muc2_room where name='">>,
      SName, <<"' and host='">>, SHost,
      <<"';">>])
      end,
  ejabberd_odbc:sql_transaction(LServer, F).
...
can_use_nick(_LServer, Host, JID, Nick, mnesia) ->
  ...
  LUS = {LUser, LServer},
  case catch mnesia:dirty_select(muc2_registered,
    [{#muc2_registered{us_host = '$1',
      nick = Nick, _ = '_'},
      [{'==', {element, 2, '$1'}, Host}],
      ['$_']}])
  of
    {'EXIT', _Reason} -> true;
    [] -> true;
    [#muc2_registered{us_host = {U, _Host}}] -> U == LUS
  end;
can_use_nick(LServer, Host, JID, Nick, riak) ->
  {LUser, LServer, _} = jid:tolower(JID),
  LUS = {LUser, LServer},
  case ejabberd_riak:get_by_index(muc2_registered,
    muc2_registered_schema(),
    <<"nick_host">>, {Nick, Host}) of
    {ok, []} ->
      true;
    {ok, [#muc2_registered{us_host = {U, _Host}}]} ->
      U == LUS;
    {error, _} ->
      true
  end;
can_use_nick(LServer, Host, JID, Nick, odbc) ->
  ...
  SHost = ejabberd_odbc:escape(Host),
  case catch ejabberd_odbc:sql_query(LServer,
    [<<"select jid from muc2_registered ">>,
      <<"where nick='">>, SNick,
      <<"' and host='">>, SHost, <<"';">>])
  of
...
init([Host, Opts]) ->
  ...
  case gen_mod:db_type(Host, Opts) of
    mnesia ->
      mnesia:create_table(muc2_room,
        [{disc_copies, [node()]},
          {attributes,
            record_info(fields, muc2_room)}]),
      mnesia:create_table(muc2_registered,
        [{disc_copies, [node()]},
          {attributes,
            record_info(fields, muc2_registered)}]),
      update_tables(MyHost),
      mnesia:add_table_index(muc2_registered, nick);
    _ ->
      ok
  end,
  mnesia:create_table(muc2_online_room,
    [{ram_copies, [node()]},
      {attributes, record_info(fields, muc2_online_room)}]),
  mnesia:add_table_copy(muc2_online_room, node(), ram_copies),
...
handle_call({create, Room, From, Nick, Opts}, _From,
  ...
  ?DEBUG("MUC2: create new room '~s'~n", [Room]),
  NewOpts = case Opts of
              default -> DefOpts;
              _ -> Opts
            end,
  {ok, Pid} = mod_muc2_room:start(
...
handle_info({room_destroyed, RoomHost, Pid}, State) ->
  F = fun () ->
    mnesia:delete_object(#muc2_online_room{name_host =
    RoomHost,
      pid = Pid})
      end,
  mnesia:transaction(F),
...
do_route1(Host, ServerHost, Access, HistorySize, RoomShaper,
  ...
  case Room of
            ...
            <<"iq">> ->
              case jlib:iq_query_info(Packet) of
                ...
                #iq{type = get, xmlns = (?NS_VCARD) = XMLNS,
                  ...
                  ejabberd_router:route(To, From,
                    jlib:iq_to_xml(Res));
                #iq{type = get, xmlns = ?NS_MUC2_UNIQUE} = IQ ->
                  Res = IQ#iq{type = result,
                    sub_el =
                    [#xmlel{name = <<"unique">>,
                      attrs =
    ...
    _ ->
      case mnesia:dirty_read(muc2_online_room, {Room, Host}) of
        [] ->
            ...
            {<<"presence">>, <<"">>} ->
              case check_user_can_create_room(ServerHost,
                ...
                true ->
                  ...
                  register_room(Host, Room, Pid),
                  mod_muc2_room:route(Pid, From, Nick, Packet),
                  ok;
                false ->
                  Lang = fxml:get_attr_s(<<"xml:lang">>, Attrs),
        ...
        [R] ->
          Pid = R#muc2_online_room.pid,
          ?DEBUG("MUC2: send to process ~p~n", [Pid]),
          mod_muc2_room:route(Pid, From, Nick, Packet),
          ok
      end
  end.
...
get_rooms(_LServer, Host, mnesia) ->
  case catch mnesia:dirty_select(muc2_room,
    [{#muc2_room{name_host = {'_', Host},
      _ = '_'},
      [], ['$_']}])
  of
...
get_rooms(_LServer, Host, riak) ->
  case ejabberd_riak:get(muc2_room, muc2_room_schema()) of
    {ok, Rs} ->
      lists:filter(
        fun(#muc2_room{name_host = {_, H}}) ->
          Host == H
        end, Rs);
    _Err ->
      []
...
get_rooms(LServer, Host, odbc) ->
  SHost = ejabberd_odbc:escape(Host),
  case catch ejabberd_odbc:sql_query(LServer,
    [<<"select name, opts from muc2_room ">>,
      <<"where host='">>, SHost, <<"';">>])
  of
    {selected, [<<"name">>, <<"opts">>], RoomOpts} ->
      lists:map(fun ([Room, Opts]) ->
        #muc2_room{name_host = {Room, Host},
          opts = opts_to_binary(
...
load_permanent_rooms(Host, ServerHost, Access,
  ...
  lists:foreach(
    fun(R) ->
      {Room, Host} = R#muc2_room.name_host,
      case mnesia:dirty_read(muc2_online_room, {Room, Host}) of
        [] ->
          {ok, Pid} = mod_muc2_room:start(Host,
...
start_new_room(Host, ServerHost, Access, Room,
  ...
  case restore_room(ServerHost, Host, Room) of
    error ->
      ?DEBUG("MUC2: open new room '~s'~n", [Room]),
      mod_muc2_room:start(Host, ServerHost, Access, Room,
        HistorySize, RoomShaper,
        From, Nick, DefRoomOpts);
    Opts ->
      ?DEBUG("MUC2: restore room '~s'~n", [Room]),
      mod_muc2_room:start(Host, ServerHost, Access, Room,
...
register_room(Host, Room, Pid) ->
  F = fun() ->
    mnesia:write(#muc2_online_room{name_host = {Room, Host},
      pid = Pid})
      end,
  mnesia:transaction(F).
...
iq_disco_info(ServerHost, Lang) ->
  [#xmlel{name = <<"identity">>,
    ...
    #xmlel{name = <<"feature">>,
      attrs = [{<<"var">>, ?NS_MUC2}], children = []},
    #xmlel{name = <<"feature">>,
      attrs = [{<<"var">>, ?NS_MUC2_UNIQUE}], children = []},
    #xmlel{name = <<"feature">>,
...
iq_disco_items_list(Host, Rooms, Query) ->
  lists:zf(fun (#muc2_online_room{name_host =
  {Name, _Host},
    pid = Pid}) ->
...
get_vh_rooms(Host, #rsm_in{max=M, direction=Direction, id=I, index=Index})->
  ...
  L = lists:sort(
    mnesia:dirty_select(muc2_online_room,
      [{#muc2_online_room{name_host = '$1', _ = '_'},
        Guard,
        ['$_']}])),
  L2 = if
  ...
  if L2 == [] -> {L2, #rsm_out{count = Count}};
    true ->
      ...
      T = lists:last(L2),
      {F, _} = H#muc2_online_room.name_host,
      {Last, _} = T#muc2_online_room.name_host,
      {L2,
...
%% @spec (Desired::muc2_online_room(), Rooms::[muc2_online_room()]) -> integer()
get_room_pos(Desired, Rooms) ->
  get_room_pos(Desired, Rooms, 0).

get_room_pos(Desired, [HeadRoom | _], HeadPosition)
  when Desired#muc2_online_room.name_host ==
  HeadRoom#muc2_online_room.name_host ->
  HeadPosition;
...
get_nick(_LServer, Host, From, mnesia) ->
  ...
  LUS = {LUser, LServer},
  case catch mnesia:dirty_read(muc2_registered,
    {LUS, Host})
  of
    {'EXIT', _Reason} -> error;
    [] -> error;
    [#muc2_registered{nick = Nick}] -> Nick
  end;
get_nick(LServer, Host, From, riak) ->
  {LUser, LServer, _} = jid:tolower(From),
  US = {LUser, LServer},
  case ejabberd_riak:get(muc2_registered,
    muc2_registered_schema(),
    {US, Host}) of
    {ok, #muc2_registered{nick = Nick}} -> Nick;
    {error, _} -> error
  end;
get_nick(LServer, Host, From, odbc) ->
  ...
  SHost = ejabberd_odbc:escape(Host),
  case catch ejabberd_odbc:sql_query(LServer,
    [<<"select nick from muc2_registered where "
    "jid='">>,
      SJID, <<"' and host='">>, SHost,
      <<"';">>])
  of
...
set_nick(_LServer, Host, From, Nick, mnesia) ->
  ...
  F = fun () ->
    case Nick of
      <<"">> ->
        mnesia:delete({muc2_registered, {LUS, Host}}), ok;
      _ ->
        Allow = case mnesia:select(muc2_registered,
          [{#muc2_registered{us_host =
          '$1',
            nick = Nick,
            _ = '_'},
            [{'==', {element, 2, '$1'},
            ...
            ['$_']}])
                of
                  [] -> true;
                  [#muc2_registered{us_host = {U, _Host}}] ->
                    U == LUS
                end,
        if Allow ->
          mnesia:write(#muc2_registered{us_host = {LUS, Host},
            nick = Nick}),
          ok;
          true -> false
        end
...
set_nick(LServer, Host, From, Nick, riak) ->
  ...
  {atomic,
    case Nick of
      <<"">> ->
        ejabberd_riak:delete(muc2_registered, {LUS, Host});
      _ ->
        ...
        if Allow ->
          ejabberd_riak:put(#muc2_registered{us_host = {LUS, Host},
            nick = Nick},
            muc2_registered_schema(),
            [{'2i', [{<<"nick_host">>,
              {Nick, Host}}]}]);
...
set_nick(LServer, Host, From, Nick, odbc) ->
  ...
  F = fun () ->
    case Nick of
      <<"">> ->
        ejabberd_odbc:sql_query_t([<<"delete from muc2_registered where ">>,
          <<"jid='">>, SJID,
          <<"' and host='">>, Host,
          <<"';">>]),
        ok;
      _ ->
        Allow = case
                  ejabberd_odbc:sql_query_t([<<"select jid from muc2_registered ">>,
                    <<"where nick='">>,
                    SNick,
                    <<"' and host='">>,
                    SHost, <<"';">>])
        ...
        if Allow ->
          odbc_queries:update_t(<<"muc2_registered">>,
            [<<"jid">>, <<"host">>,
              <<"nick">>],
            [SJID, SHost, SNick],
            [<<"jid='">>, SJID,
...
iq_get_vcard(Lang) ->
  [#xmlel{name = <<"FN">>, attrs = [],
    ...
    #xmlel{name = <<"DESC">>, attrs = [],
      ...
      [{xmlcdata,
        <<(translate:translate(Lang,
          <<"ejabberd MUC2 module">>))/binary,
          "\nCopyright (c) 2003-2016 ProcessOne">>}]}].

broadcast_service_message(Host, Msg) ->
  lists:foreach(
    fun(#muc2_online_room{pid = Pid}) ->
...
update_tables(Host) ->
  update_muc2_room_table(Host),
  update_muc2_registered_table(Host).

muc2_room_schema() ->
  {record_info(fields, muc2_room), #muc2_room{}}.

muc2_registered_schema() ->
  {record_info(fields, muc2_registered), #muc2_registered{}}.

update_muc2_room_table(_Host) ->
  Fields = record_info(fields, muc2_room),
  case mnesia:table_info(muc2_room, attributes) of
    Fields ->
      ejabberd_config:convert_table_to_binary(
        muc2_room, Fields, set,
        fun(#muc2_room{name_host = {N, _}}) -> N end,
        fun(#muc2_room{name_host = {N, H},
          opts = Opts} = R) ->
          R#muc2_room{name_host = {iolist_to_binary(N),
            iolist_to_binary(H)},
            opts = opts_to_binary(Opts)}
        end);
    _ ->
      ?INFO_MSG("Recreating muc2_room table", []),
      mnesia:transform_table(muc2_room, ignore, Fields)
  end.

update_muc2_registered_table(_Host) ->
  Fields = record_info(fields, muc2_registered),
  case mnesia:table_info(muc2_registered, attributes) of
    Fields ->
      ejabberd_config:convert_table_to_binary(
        muc2_registered, Fields, set,
        fun(#muc2_registered{us_host = {_, H}}) -> H end,
        fun(#muc2_registered{us_host = {{U, S}, H},
          nick = Nick} = R) ->
          R#muc2_registered{us_host = {{iolist_to_binary(U),
            iolist_to_binary(S)},
            iolist_to_binary(H)},
            nick = iolist_to_binary(Nick)}
        end);
    _ ->
      ?INFO_MSG("Recreating muc2_registered table", []),
      mnesia:transform_table(muc2_registered, ignore, Fields)
  end.

export(_Server) ->
  [{muc2_room,
    fun(Host, #muc2_room{name_host = {Name, RoomHost}, opts = Opts}) ->
      case str:suffix(Host, RoomHost) of
        true ->
          SName = ejabberd_odbc:escape(Name),
          SRoomHost = ejabberd_odbc:escape(RoomHost),
          SOpts = ejabberd_odbc:encode_term(Opts),
          [[<<"delete from muc2_room where name='">>, SName,
            <<"' and host='">>, SRoomHost, <<"';">>],
            [<<"insert into muc2_room(name, host, opts) ",
              "values (">>,
              <<"'">>, SName, <<"', '">>, SRoomHost,
              <<"', '">>, SOpts, <<"');">>]];
        false ->
    ...
    {muc2_registered,
      fun(Host, #muc2_registered{us_host = {{U, S}, RoomHost},
        nick = Nick}) ->
        case str:suffix(Host, RoomHost) of
          true ->
            ...
            SNick = ejabberd_odbc:escape(Nick),
            SRoomHost = ejabberd_odbc:escape(RoomHost),
            [[<<"delete from muc2_registered where jid='">>,
              SJID, <<"' and host='">>, SRoomHost, <<"';">>],
              [<<"insert into muc2_registered(jid, host, "
              "nick) values ('">>,
...
import(_LServer) ->
  [{<<"select name, host, opts from muc2_room;">>,
    fun([Name, RoomHost, SOpts]) ->
      Opts = opts_to_binary(ejabberd_odbc:decode_term(SOpts)),
      #muc2_room{name_host = {Name, RoomHost}, opts = Opts}
    end},
    {<<"select jid, host, nick from muc2_registered;">>,
      fun([J, RoomHost, Nick]) ->
        #jid{user = U, server = S} =
          jid:from_string(J),
        #muc2_registered{us_host = {{U, S}, RoomHost},
          nick = Nick}
      end}].

import(_LServer, mnesia, #muc2_room{} = R) ->
  mnesia:dirty_write(R);
import(_LServer, mnesia, #muc2_registered{} = R) ->
  mnesia:dirty_write(R);
import(_LServer, riak, #muc2_room{} = R) ->
  ejabberd_riak:put(R, muc2_room_schema());
import(_LServer, riak,
    #muc2_registered{us_host = {_, Host}, nick = Nick} = R) ->
  ejabberd_riak:put(R, muc2_registered_schema(),
