...
%%%
%%% @end
%%% Created : 10. Mar 2016 9:21 PM
%%%-------------------------------------------------------------------
-module(mod_muc2_room).
...
init([Host, ServerHost, Access, Room, HistorySize,
  ...
  if (State1#state.config)#config.persistent ->
    mod_muc2:store_room(State1#state.server_host,
      State1#state.host,
      State1#state.room,
      make_opts(State1));
    true -> ok
        ...
        <<"groupchat">> ->
          ...
          MinMessageInterval =
            trunc(gen_mod:get_module_opt(StateData#state.server_host,
              mod_muc2, min_message_interval, fun(MMI) when is_number(MMI) -> MMI end, 0)
              * 1000000),
                  ...
                  true ->
                    case get_affiliation(IJID, StateData) of
                      none ->
                        ...
                        of
                          true ->
                            mod_muc2:store_room(NSD#state.server_host,
                              NSD#state.host,
                              NSD#state.room,
                              make_opts(NSD));
                          _ -> ok
...
normal_state({route, From, Nick,
  ...
  MinPresenceInterval =
    trunc(gen_mod:get_module_opt(StateData#state.server_host,
      mod_muc2, min_presence_interval,
...
terminate(Reason, _StateName, StateData) ->
  ...
  mod_muc2:room_destroyed(StateData#state.host, StateData#state.room, self(),
    StateData#state.server_host),
  ok.
      ...
      #xmlel{attrs = Attrs} = Packet ->
        ...
        StateData1 = case Type of
                       <<"unavailable">> ->
                         ...
                         case To of
                           <<"">> ->
                             ?DEBUG("process_presence unavailable empty To, ~p, ~p, ~p, To: ~p", [Packet, From, Nick, To]),
                             StateData;
                           _ ->
                             ?DEBUG("process_presence unavailable with To ~p, ~p, ~p, To: ~p", [Packet, From, Nick, To]),
                             NewPacket = case
                             ...
                             case is_nick_change(From, Nick, StateData) of
                               true ->
                                 case {nick_collision(From, Nick, StateData),
                                   mod_muc2:can_use_nick(StateData#state.server_host,
...
expulse_participant(Packet, From, StateData, Reason1) ->
  ?DEBUG("expulse_participant ~p, ~p, ~p", [Packet, From, Reason1]),
...
get_max_users_admin_threshold(StateData) ->
  gen_mod:get_module_opt(StateData#state.server_host,
    mod_muc2, max_users_admin_threshold,
    fun(I) when is_integer(I), I>0 -> I end,
    5).
    ...
    error ->
      MessageShaper =
        shaper:new(gen_mod:get_module_opt(StateData#state.server_host,
          mod_muc2, user_message_shaper,
          fun(A) when is_atom(A) -> A end,
          none)),
      PresenceShaper =
        shaper:new(gen_mod:get_module_opt(StateData#state.server_host,
          mod_muc2, user_presence_shaper,
          fun(A) when is_atom(A) -> A end,
          none)),
    ...
    case El of
      ...
      #xmlel{attrs = Attrs1} ->
        XMLNS = fxml:get_attr_s(<<"xmlns">>,
          Attrs1),
        NS_MUC2 = ?NS_MUC2,
        Size = byte_size(NS_MUC2),
        case XMLNS of
          <<NS_MUC2:Size/binary, _/binary>> ->
            false;
          _ ->
            true
        end
...
add_user_presence(JID, Presence, StateData) ->
  ...
  Users = (?DICT):update(LJID,
    ...
    StateData#state.users),
  StateData1 = StateData#state{users = Users},
  mod_muc2:store_room(StateData1#state.server_host,
...
send_new_presence1(NJID, Reason, IsInitialPresence, StateData, OldStateData) ->
  ...
  lists:foreach(fun ({LUJID, Info}) ->
    ...
    StatusEls = status_els(IsInitialPresence, NJID, Info,
      StateData),
    #xmlel{attrs = Attrs} = Presence,
    Message = case fxml:get_attr_s(<<"type">>, Attrs) of
      <<"">> ->
        #xmlel{name = <<"message">>,
      ...
      PresenceType ->
        #xmlel{name = <<"message">>,
          ...
          children = [#xmlel{name = <<"body">>,
            children = [{xmlcdata, PresenceType}]}]}
    end,
    Packet = fxml:append_subtags(Message,
      [#xmlel{name = <<"x">>,
        attrs =
        [{<<"xmlns">>,
          ?NS_MUC2_USER}],
...
change_config(Config, StateData) ->
  ...
  of
    ...
    {true, false} ->
      mod_muc2:forget_room(NSD#state.server_host,
        NSD#state.host, NSD#state.room);
    {false, false} -> ok
  end,
...
set_opts([{Opt, Val} | Opts], StateData) ->
  NSD = case Opt of
          ...
          users ->
            StateData#state{users = (?DICT):from_list(Val)};
          nicks ->
            StateData#state{nicks = (?DICT):from_list(Val)};
          _ -> StateData
        end,
  set_opts(Opts, NSD).
...
destroy_room(DEl, StateData) ->
  ...
  case (StateData#state.config)#config.persistent of
    true ->
      mod_muc2:forget_room(StateData#state.server_host,
...
check_decline_invitation(Packet) ->
  ...
  (?NS_MUC2_USER) = fxml:get_tag_attr_s(<<"xmlns">>, XEl),
...
tab_add_online_user(JID, StateData) ->
  ...
  Host = StateData#state.host,
    catch ets:insert(muc2_online_users,
    #muc2_online_users{us = US, resource = LResource,
      room = Room, host = Host}).
...
tab_remove_online_user(JID, StateData) ->
  ...
  Room = StateData#state.room,
  Host = StateData#state.host,
    catch ets:delete_object(muc2_online_users,
    #muc2_online_users{us = US, resource = LResource,
      room = Room, host = Host}).
...
tab_count_user(JID) ->
  ...
  US = {LUser, LServer},
  case catch ets:select(muc2_online_users,
    [{#muc2_online_users{us = US, _ = '_'}, [], [[]]}])
  of
...
send_multiple(From, Server, Users, Packet) ->
  JIDs = [jlib:jid_remove_resource(User#user.jid) || {_, User} <- ?DICT:to_list(Users)],
